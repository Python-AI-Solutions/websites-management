...
DEFAULT_ALLOWED_CIDRS = ["10.99.0.0/24"]
...
def prompt_allowed_cidrs():
    default_text = ", ".join(DEFAULT_ALLOWED_CIDRS)
    raw = prompt("CIDRs to route through WireGuard (comma-separated)", default=default_text)
    entries = [segment.strip() for segment in raw.split(",") if segment.strip()]
    return entries if entries else list(DEFAULT_ALLOWED_CIDRS)
...
def write_client_config(peer_name, private_key, server_key, endpoint, allowed_ip, port, allowed_cidrs):
    conf_path = CONFIG_DIR / f"{peer_name}.conf"
    allowed_ips_text = ", ".join(allowed_cidrs)
...
def main():
    ...
    allowed_cidrs = prompt_allowed_cidrs()
    conf_path = write_client_config(
        peer_name, private_key, server_key, endpoint_ip, allowed_ip, listen_port, allowed_cidrs
    )
