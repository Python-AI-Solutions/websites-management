...
**Admin SSH alias:** Terraform connects to the host via the `bastion-admin` SSH config entry (`var.jump_host_admin_host`). Ensure your `~/.ssh/config` contains something like:
...
```bash
scp scripts/wireguard-bootstrap.sh bastion-admin:~/wireguard-bootstrap.sh
