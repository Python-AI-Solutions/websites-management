...
resource "null_resource" "k8s_init" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "  if grep -q 'initialDelaySeconds: 60' /etc/kubernetes/manifests/etcd.yaml && grep -q 'initialDelaySeconds: 60' /etc/kubernetes/manifests/kube-apiserver.yaml; then",
      "    echo '✅ Patches successfully applied! Probe delays set to 60s'",
      "  else",
