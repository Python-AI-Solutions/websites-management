...
resource "null_resource" "install_addons" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo helm repo update",
      "sudo helm install traefik traefik/traefik --namespace traefik --version ${var.traefik_chart_version} --kubeconfig /etc/kubernetes/admin.conf --wait=false --set ports.web.hostPort=80 --set ports.websecure.hostPort=443 --set deployment.kind=DaemonSet --set ingressClass.enabled=true --set ingressClass.isDefaultClass=true --set providers.kubernetesIngress.enabled=true",
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo helm repo update",
      "sudo helm install cert-manager jetstack/cert-manager --namespace cert-manager --version ${var.cert_manager_chart_version} --kubeconfig /etc/kubernetes/admin.conf --set installCRDs=true --wait=false",
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo helm repo update",
      "sudo helm install local-path-provisioner containeroo/local-path-provisioner --namespace local-path-storage --version ${var.local_path_provisioner_chart_version} --kubeconfig /etc/kubernetes/admin.conf --wait=false --set storageClass.defaultClass=true --set storageClass.name=local-path",
