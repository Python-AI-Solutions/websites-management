...
resource "null_resource" "k8s_init" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "  echo 'Running kubeadm init to bootstrap control plane via staged phases...'",
      "  sudo kubeadm reset -f || true",
      "  echo 'Phase 1: Initialize etcd first'",
      "  sudo kubeadm init phase etcd --config /tmp/kubeadm-config.yaml",
      "  echo 'Waiting for etcd to become leader and fully ready...'",
      "  for i in {1..15}; do",
      "    if sudo etcdctl endpoint health --endpoints=127.0.0.1:2379 --cacert=/etc/kubernetes/pki/etcd/ca.crt --cert=/etc/kubernetes/pki/etcd/server.crt --key=/etc/kubernetes/pki/etcd/server.key 2>/dev/null | grep -q healthy; then",
