...
resource "null_resource" "install_addons" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "TRAEFIK_EXISTS=$(sudo helm list --namespace traefik --kubeconfig /etc/kubernetes/admin.conf 2>/dev/null | grep -c '^traefik' || echo '0')",
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "CERT_MANAGER_EXISTS=$(sudo helm list --namespace cert-manager --kubeconfig /etc/kubernetes/admin.conf 2>/dev/null | grep -c '^cert-manager' || echo '0')",
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "LPP_EXISTS=$(sudo helm list --namespace local-path-storage --kubeconfig /etc/kubernetes/admin.conf 2>/dev/null | grep -c '^local-path-provisioner' || echo '0')",
